jQuery(document).ready(function ($) {
    ...
    $(window).scroll(function () {
        if ($(this).scrollTop() > 0) {
            $('.cs-site-header').addClass("active");
        } else {
            $('.cs-site-header').removeClass("active");
        }
    });

    // Scroll to top on click
    $('.cs-go-top').click(function () {
        $('html, body').animate({ scrollTop: 0 }, 800);
        return false;
    });

    // Header Search
    $(".cs-search-btn, .cs-close-search").click(function () {
        $(".cs-search-box").toggleClass("active")
    })
});